...
RAW_LABEL="${1:-${PLUGIN_NAME:-${APP_LABEL:-sample_utility}}}"
RAW_OUTPUT="${2:-${OUTPUT_FILENAME:-${FILE_NAME:-bundle.zip}}}"
SKIP_RESOURCES="${3:-${SKIP_RESOURCES:-false}}"

APP_LABEL="$RAW_LABEL"
SLUG=$(normalize_slug "$RAW_LABEL")
OUTPUT_FILE=$(normalize_filename "$RAW_OUTPUT")
...
# 4. Resolve Target Module Source & Resources based on App Label / Slug
echo "🎯 Resolving target module for '$APP_LABEL' (Slug: '$SLUG')..."
...
if [ -d "plugins/$SLUG/src" ]; then
    ...
    echo "❌ ERROR: Could not locate source directory for '$APP_LABEL' (looked for 'plugins/$SLUG' and 'plugin_src/')."
    exit 1
fi
...
# 7. Convert Bytecode to Android DEX via d8 with full desugaring classpath
